...
class MMLinear(sl.WorkflowTask):
    ...
    def workflow(self):
        ...
        # --------------------------------------------------------------------------------
        sample_train_and_test = self.new_task('sample_trn%s_tst%s_c%s' % (self.train_size, self.test_size, self.lin_cost), SampleTrainAndTest,
        ...
        # --------------------------------------------------------------------------------
        create_sparse_train_dataset = self.new_task('create_sparse_traindata_trn%s_tst%s_c%s' % (self.train_size, self.test_size, self.lin_cost), CreateSparseTrainDataset,
                dataset_name = self.dataset_name,
                replicate_id = self.replicate_id,
                slurminfo = sl.SlurmInfo(
                    runmode=runmode,
        ...
        # ------------------------------------------------------------------------
        create_sparse_test_dataset = self.new_task('create_sparse_testdata_trn%s_tst%s_c%s' % (self.train_size, self.test_size, self.lin_cost), CreateSparseTestDataset,
                dataset_name = self.dataset_name,
                replicate_id = self.replicate_id,
                slurminfo = sl.SlurmInfo(
                    runmode=runmode,
                    ...
                    cores='16',
                    time='1-00:00:00',
                    jobname='sparse_trn%s_tst%s_c%s' % (self.train_size, self.test_size, self.lin_cost),
                    threads='16'
                ))
        ...
        # ------------------------------------------------------------------------
        ungzip_testdata = self.new_task('ungzip_testdata_trn%s_tst%s_c%s' % (self.train_size, self.test_size, self.lin_cost), UnGzipFile,
                slurminfo = sl.SlurmInfo(
                    runmode=runmode,
                    project=self.slurm_project,
                    partition='core',
                    cores='1',
                    time='1:00:00',
                    jobname='ungziptest_trn%s_tst%s_c%s' % (self.train_size, self.test_size, self.lin_cost),
                    threads='1'
                ))
        ungzip_testdata.in_gzipped = create_sparse_test_dataset.out_sparse_testdata
        # ------------------------------------------------------------------------
        ungzip_traindata = self.new_task('ungzip_traindata_trn%s_tst%s_c%s' % (self.train_size, self.test_size, self.lin_cost), UnGzipFile,
                slurminfo = sl.SlurmInfo(
                    runmode=runmode,
                    project=self.slurm_project,
                    partition='core',
                    cores='1',
                    time='1:00:00',
                    jobname='ungziptrain_trn%s_tst%s_c%s' % (self.train_size, self.test_size, self.lin_cost),
                    threads='1'
                ))
