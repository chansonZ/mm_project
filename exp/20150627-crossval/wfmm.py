...
class MMWorkflow(sl.WorkflowTask):
    ...
    def workflow(self):
        ...
        for train_size in train_sizes:
            ...
            # --------------------------------------------------------------------------------
            create_unique_sign_copy = self.new_task('create_unique_sign_copy_%s' % self.replicate_id, CreateReplicateCopy,
                    replicate_id = self.replicate_id)
            create_unique_sign_copy.in_file = gen_sign_filter_subst.out_signatures
            # --------------------------------------------------------------------------------
            sample_train_and_test = self.new_task('sample_trn%s_tst%s_c%s' % (train_size, self.test_size, self.lin_cost), SampleTrainAndTest,
            ...
            # --------------------------------------------------------------------------------
            create_sparse_train_dataset = self.new_task('create_sparse_traindata_trn%s_tst%s_c%s' % (train_size, self.test_size, self.lin_cost), CreateSparseTrainDataset,
                    dataset_name = self.dataset_name,
                    replicate_id = self.replicate_id,
                    slurminfo = sl.SlurmInfo(
                        runmode=runmode,
            ...
            # ------------------------------------------------------------------------
            create_sparse_test_dataset = self.new_task('create_sparse_testdata_trn%s_tst%s_c%s' % (train_size, self.test_size, self.lin_cost), CreateSparseTestDataset,
                    dataset_name = self.dataset_name,
                    replicate_id = self.replicate_id,
                    slurminfo = sl.SlurmInfo(
                        runmode=runmode,
            ...
            # ------------------------------------------------------------------------
            ungzip_testdata = self.new_task('ungzip_testdata_trn%s_tst%s_c%s' % (train_size, self.test_size, self.lin_cost), UnGzipFile,
                    slurminfo = sl.SlurmInfo(
                        runmode=runmode,
                        project=self.slurm_project,
                        partition='core',
            ...
            # ------------------------------------------------------------------------
            ungzip_traindata = self.new_task('ungzip_traindata_trn%s_tst%s_c%s' % (train_size, self.test_size, self.lin_cost), UnGzipFile,
                    slurminfo = sl.SlurmInfo(
                        runmode=runmode,
                        project=self.slurm_project,
                        partition='core',
            ...
            if self.train_method == TRAINMETHOD_LIBLINEAR:
                train_model = self.new_task('train_lin_trn%s_tst%s_c%s' % (train_size, self.test_size, self.lin_cost), TrainLinearModel,
                        replicate_id = self.replicate_id,
                        dataset_name = self.dataset_name,
                        train_size = train_size,
                        test_size = self.test_size,
                ...
                # ------------------------------------------------------------------------
                predict = self.new_task('predict_lin_trn%s_tst%s_c%s' % (train_size, self.test_size, self.lin_cost), PredictLinearModel,
                        dataset_name = self.dataset_name,
                        replicate_id = self.replicate_id,
                        slurminfo = sl.SlurmInfo(
                            runmode=runmode,
                ...
                # ------------------------------------------------------------------------
                assess_model = self.new_task('assess_lin_trn%s_tst%s_c%s' % (train_size, self.test_size, self.lin_cost), AssessLinearRMSD,
                        dataset_name = self.dataset_name,
                        replicate_id = self.replicate_id,
                        lin_cost = self.lin_cost,
                        slurminfo = sl.SlurmInfo(
            ...
            # ========================================================================
                train_model = self.new_task('train_svm_trn%s_tst%s_g%s_c%s' % (train_size, self.test_size, self.svm_gamma, self.svm_cost), TrainSVMModel,
                        replicate_id = self.replicate_id,
                        dataset_name = self.dataset_name,
                        train_size = train_size,
                        svm_gamma = '0.001',
                ...
                train_model.in_traindata = ungzip_traindata.out_ungzipped
                # ------------------------------------------------------------------------
                predict = self.new_task('predict_svm_trn%s_tst%s_g%s_c%s' % (train_size, self.test_size, self.svm_gamma, self.svm_cost), PredictSVMModel,
                        dataset_name = self.dataset_name,
                        replicate_id = self.replicate_id,
                        slurminfo = sl.SlurmInfo(
                            runmode=runmode,
                ...
                predict.in_sparse_testdata = ungzip_testdata.out_ungzipped
                # ------------------------------------------------------------------------
                assess_model = self.new_task('assess_lin_trn%s_tst%s_c%s' % (train_size, self.test_size, self.lin_cost),
                        AssessSVMRMSD,
                        dataset_name = self.dataset_name,
                        replicate_id = self.replicate_id,
